...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \headerbox{What is topic hierarchy}{name=problem,column=0,row=0}{
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   }

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \headerbox{Base topic model}{name=artm,column=0, span=1}{
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\textbf{Given:}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\headerbox{Base EM algorithm}{name=baseem, column=1, row=0}{
 ...
 \smallskip
   ...
   $r_{wt} =  \phi_{wt}\dfrac{\partial R}{\partial\phi_{wt}}$, $r_{td} =  \theta_{td}\dfrac{\partial R}{\partial\theta_{td}}$

    $\phi_{wt}   \propto \biggl(n_{wt} + r_{wt} \biggr)_{\!+}$, $\theta_{td} \propto  \biggl(n_{td} + r_{td}\biggr)_{\!+}$
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    Replacing $\phi_{wt}$ and $\theta_{td}$ with their unbiased maximum likelihood estimators in M-step one can obtain the following formulas:
...
Denote $x^k$ as value of $x$ on $k$-th iteration.
 Suppose that
\begin{enumerate}
\item  $ n^k_{wt} = 0 \Rightarrow \phi^k_{wt} = 0$. 
\item $n^k_{td} = 0 \Rightarrow \theta^k_{td} = 0$.
\item $\exists \varepsilon>0\ \exists N\ \forall k > N\ \phi^k_{wt}, \theta^k_{td} \notin (0, \varepsilon)$.
\item  $ n_{dw}>0 \Rightarrow \forall k\ \exists t\colon p^k_{tdw} > 0$.
\item $\exists \delta\geq 0\ \exists N\ \forall k > N \ \forall t\ \exists w\  n^k_{wt} + r^k_{wt} > \delta$ and the similar condition for $\theta$.
\item $\exists N\ \forall k > N\colon\ \ Q^k (\phi^k, \theta^k)+ R(\phi^k, \theta^k) \geq Q^k(\phi^{k-1}, \theta^{k-1}) + R(\phi^{k-1}, \theta^{k-1})$, where $Q^k(\phi, \theta) = \sum\limits_{t,d,w} p^k_{tdw} (\ln \phi_{wt} + \ln \theta_{td})$.
\end{enumerate}
Then for  $k \to \infty$
\[
|p_{tdw}^{k} - p_{tdw}^{k-1}| \to 0.
\]
And if $\delta > 0$ then for $k \to \infty$
\[
\|\phi^k - \phi^{k-1}\|_1 \to 0, \|\theta^k - \theta^{k-1}\|_1 \to 0 .
\]
  }
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\headerbox{M-step properties}{name=quality,column=2,below=em}{
Consider $R^{\prime}(n_{wt}, n_{td}) = R(\frac{n_{wt}}{\sum\limits_w n_{wt}}, \frac{n_{td}}{\sum\limits_t n_{td}})$.\\
In this case
\smallskip
\smallskip

$\frac{\partial{R^{\prime}}}{\partial{n_{wt}}} =\frac{1}{\sum\limits_w n_{wt}} \big(  \frac{\partial{R}}{\partial{\phi_{wt}}} - \sum\limits_u \phi_{ut} \frac{\partial{R}}{\partial{\phi_{ut}}} \big)$,

$\frac{\partial{R^{\prime}}}{\partial{n_{td}}} =\frac{1}{\sum\limits_t n_{td}} \big(  \frac{\partial{R}}{\partial{\theta_{td}}} - \sum\limits_s \theta_{sd} \frac{\partial{R}}{\partial{\theta_{sd}}} \big)$.
\smallskip

Under conditions of the convergence theorem and with the unbiased modification of M-step  one can show that:
\[
\sum_{w, t} (n_{wt}^{k+1} - n_{wt}^{k})  \frac{\partial{R^{\prime}}}{\partial{n_{wt}}} =
\]
\[
=  \frac12 \sum\limits_{t, w, u}   \frac{1}{\sum\limits_v n_{vt}} \bigg(  \frac{\partial{R}}{\partial{\phi_{wt}}}  -  \frac{\partial{R}}{\partial{\phi_{ut}}}  \bigg)^2 \phi_{wt} \phi_{ut}  \geq 0
\]
and similar formula for $\theta$.
 }

  \headerbox{Gradient Modification}{name=eme,column=2,below=quality}{
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
One suggests to use the gradient of $R^{\prime}$ as regularization additives. It results in the following M-step:
\smallskip
\smallskip
...
\headerbox{References}{name=references,column=0,below=pictures,span=3}{
    \textit{Vorontsov K. V., Potapenko A. A.} Tutorial on Probabilistic Topic Modeling:
    Additive Regularization for Stochastic Matrix Factorization. ---
    Analysis of Images, Social Networks, and Texts (AIST-2014). --- LNCS, Springer.
}
