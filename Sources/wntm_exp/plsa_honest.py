...
def custom_reg(origin_n_tw, origin_n_dt):
    n_tw = np.copy(origin_n_tw)

    n_w = np.sum(n_tw, axis=0)
    n_tw[np.sum(n_tw, axis=1) < 1e-20, :] += 1
    
    theta_matrix = n_tw / np.sum(n_tw, axis=1)[:, np.newaxis]
    theta_matrix[:, np.sum(phi_matrix, axis=0) < 1e-20] += 1
    theta_matrix /= np.sum(phi_matrix, axis=0)

    ans = - n_w * theta_matrix / 2.
    return ans, - origin_n_dt  + np.transpose(origin_n_tw + ans)
...
for seed in xrange(777, 779):
    ...
    with open('plsa_honest/exp_seed_{}.pkl'.format(seed), 'w') as f:
        pickle.dump({
            'phi': phi, 
            'theta': theta, 
            'n_tw': n_tw, 
