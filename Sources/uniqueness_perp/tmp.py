...
def investigate_phi_uniqueness(phi):
    ...
    for t in xrange(T):
        ...
        if np.sum(positions) != 0:
            print '\tSubmatrix rank', np.linalg.matrix_rank(matrix[np.ix_(topics, positions)], compute_uv=False)
            eigen_values = np.linalg.svd(matrix[np.ix_(topics, positions)], compute_uv=False)[1]
            print '\tEigen values:', eigen_values
            max_val = np.min(np.linalg.svd(matrix[topics, :], compute_uv=False)[1])
...
def calc_phi_min_singular(phi):
    return np.min(np.linalg.svd(phi, compute_uv=False)[1])
...
for T in xrange(2, 11):
    ...
    for seed in xrange(10):
        ...
        tmp_value.append(calc_phi_min_singular(phi))
    ...
    tmp_values.append(np.mean(tmp_value))
    ...
    tmp_values_err.append(3. / np.sqrt(n) * np.std(tmp_value))
    ...
    print tmp_values[-5:]
    print tmp_values_err[-5:]
    print ''
    print ''

with open('tmp_topics_dependency_origin_3.pkl', 'w') as f:
    pickle.dump({
